...
class _CreatePlayerScreenState extends State<CreatePlayerScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> createPlayer() async {
    ...
    // On Android versions that still expose the legacy storage permission,
    // show the normal system permission dialog before the first career save.
    // Saving itself remains app-private and therefore also works when Android
    // no longer exposes a generic storage permission.
    if (Platform.isAndroid) {
      await WorldSave.prepareStorageAccess();
    }
    ...
    if (!saved) {
      ...
      HapticFeedback.heavyImpact();
      unawaited(FPGAudio.playSfx(FPGAudio.error));
      final reason = WorldSave.lastError;
        ...
        SnackBar(
          ...
          content: Text(
            reason == null
                ...
                : 'Nie udało się zapisać kariery: $reason',
          ),
        ),
      );
      return;

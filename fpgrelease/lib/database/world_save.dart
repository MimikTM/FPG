...
class WorldSave {
  ...
  /// Human-readable reason the most recent save failed, if it did.
  /// UI layers (e.g. the "career start" screen) can surface this directly
  /// instead of forcing the person to dig through Settings > Diagnostics.
  static String? lastError;

  /// Recursively rewrites a JSON-ish tree so it is guaranteed encodable.
  ///
  /// `jsonEncode` throws (and previously silently failed the whole save)
  /// if ANY double anywhere in the tree is NaN or +/-Infinity — which can
  /// happen from a stray division inside one of the many simulation
  /// engines (market value, form, production factors, etc.). Rather than
  /// aborting the entire career save because of one bad number deep in an
  /// AI club's stats, we sanitize it to 0 and keep going.
  ...
  static Future<bool> _write(GameEngine engine) async {
    ...
    try {
      ...
      // Sanitize first, then encode. If the tree still somehow fails to
      // encode (e.g. a non-primitive value snuck in somewhere), fall back
      // to encoding the sanitized copy explicitly so a single bad value
      // deep in AI-world data cannot silently kill the whole save.
      String encoded;
      try {
        encoded = jsonEncode(payload);
      } catch (_) {
        encoded = jsonEncode(_sanitize(payload));
      }
      await temp.writeAsString(
        encoded,
        flush: true,
      );
      // The backup copy is best-effort. It must never take down a save
      // that otherwise succeeded (e.g. if the device is low on the disk
      // space needed to hold a second copy of the file).
  ...
  static Map<String, dynamic> _careerJson(dynamic p) => {
        ...
        'age': p.age,
        'height': p.height,
        'position': positionCodeOf(p.position),
        'overall': p.overall,

...
class MatchSituationEngine {
  ...
  MatchSituation build({
    ...
    // A normal match has 11 players per side, but degraded/corrupt rosters,
    // red cards or a future ruleset can temporarily leave one side without a
    // valid teammate/opponent. Situation generation is presentation logic and
    // must never crash the simulation in that state.
      ...
      return MatchSituation(
        ...
        beats: [
          MatchSituationBeat(
            ...
            actorId: owner.id,
            description: '${owner.name} ma wolną przestrzeń',
          ),
        ],
      );
    }
      ...
      return MatchSituation(
        ...
        beats: [
          MatchSituationBeat(
            ...
            description: '${owner.name} próbuje samodzielnie wyprowadzić akcję',
          ),
        ],
      );
    }
    ...
    final nearestDefender = _nearest(opponents, owner.x, owner.y);
    final style = TeamPlayStyleProfile.fromManagerStyle(managerStyle).style;
    ...
    if (distanceToGoal > 40 && _spaceAhead(owner, opponents, direction) > 18 &&
        random.nextDouble() < (style == TeamPlayStyle.counter || style == TeamPlayStyle.direct ? .68 : .25)) {
      return MatchSituation(
    ...
    if (style == TeamPlayStyle.wingPlay || owner.position == PlayerPosition.winger || (owner.y < 25 || owner.y > 75) && distanceToGoal < 48) {
    ...
    if (style == TeamPlayStyle.direct) {
      return MatchSituation(kind: MatchSituationKind.counter, beats: [
        MatchSituationBeat(type: Match2DEventType.pass, actorId: owner.id, secondaryId: striker.id, description: '${owner.name} zagrywa bezpośrednio za linię pomocy'),
        MatchSituationBeat(type: Match2DEventType.shot, actorId: striker.id, secondaryId: nearestDefender.id, description: '${striker.name} walczy o drugą piłkę i kończy akcję', isChance: distanceToGoal < 38),
      ]);
    }

    // Central combination: midfielder -> striker -> finish.
    if ((style == TeamPlayStyle.possession || owner.position == PlayerPosition.midfielder || owner.position == PlayerPosition.striker) && distanceToGoal < 55) {
      return MatchSituation(

...
class GlobalMatchEngine {
  ...
  void _assignGoals(List<PlayerMatchPerformance> performances, int goals, List<Player> players, List<PlayerMatchEvent> events) {
    ...
    for (final perf in performances) {
      final p = players.where((x) => x.id == perf.playerId).firstOrNull;
      // Old saves can contain a performance for a player who has since
      // retired. Ignore that orphan instead of crashing the next-day world
      // simulation while assigning scorers.
      if (p == null) continue;
    ...
    for (var i = 0; i < goals; i++) {
      // Weighted entries can point to an older immutable performance object
      // after a previous goal updated that player. Resolve the selected
      // player by id against the current list instead of using indexOf on a
      // stale object reference (which could return -1 and crash the world
      // simulation when the same scorer is selected twice).
      final selected = weighted[_random.nextInt(weighted.length)];
      final playerId = selected.playerId;
      final index = performances.indexWhere((p) => p.playerId == playerId);
      if (index < 0) continue;

...
class RelationshipWebEngine {
  ...
  /// Explicit relationship values are the persistent graph state. Calling
  /// this after a career action guarantees the same object is returned when
  /// the player leaves and re-enters a screen.
  void syncCareerPlayer(Player p) {
    final r = forPlayer(p);
    // Keep the world projection compatible with the graph without replacing
    // the graph's explicit values. Only the corresponding base football
    // attributes are nudged toward the relationship state.
    p.managerRelationship = r.coach;
    p.fanSupport = r.fans;
    p.agentInfluence = r.agent;
    p.mediaPressure = ((r.media - 50) * 2).clamp(0, 100).toInt();
  }
  ...
  List<WorldEvent> processDay({
    ...
    for (final p in players) {
      ...
      final previousUpdate = r.lastUpdatedAbsoluteDay;
      r.lastUpdatedAbsoluteDay = absoluteDay;
      final allowDrift = previousUpdate <= 0 || absoluteDay - previousUpdate >= 3;

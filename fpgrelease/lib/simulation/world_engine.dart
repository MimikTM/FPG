...
import 'market_value_engine.dart';
import 'world_integrity_validator.dart';
import 'squad_depth_engine.dart';
...
class WorldEngine {
  ...
  late final MarketValueEngine marketValueEngine;
  final Map<String, List<Fixture>> fixturesByLeague = {};
  final Map<String, Map<String, Standing>> standingsByLeague = {};

  // PERF: O(1) lookup indexes, rebuilt every time _syncClubRosters() runs
  // (i.e. every point in the day where players/clubs may have changed,
  // including right after transfers). Before this, _findClub() scanned the
  // full club list and _playersOfClub() scanned the FULL player list for
  // every single call — with ~56 clubs and ~1700 players, called for every
  // match/every club/every day, that is what produced the multi-minute
  // "advance day" freeze. Both are now backed by these maps instead.
  Map<String, Club> _clubIndex = {};
  Map<String, Player> _playerIndex = {};
  ...
  WorldEngine({
    ...
    marketValueEngine = MarketValueEngine();
    _initializeGlobalLeagues();
    // Build the id indexes immediately so _findClub()/_playersOfClub() are
    // safe to call even before the first processDay() (e.g. right after
    // loading a save or starting a new career).
    _syncClubRosters();
  }
  ...
  void processDay({
    ...
    // V26: market value is refreshed AFTER real match production and BEFORE
    // scouting/transfer decisions. The market therefore prices what players
    // actually did, not stale OVR-only values.
    for (final player in players) {
      marketValueEngine.refreshWorldPlayer(player);
    }
    ...
    // Wypożyczenia rozliczamy na podstawie absolutnego dnia świata.
    final loanLogs = transferEngine.processLoanReturns(
      clubs: clubs, players: players, absoluteDay: absoluteDay,
    );
    if (summerTransferWindow || winterTransferWindow) {
      loanLogs.addAll(transferEngine.processWindow(
        clubs: clubs,
        players: players,
        summer: summerTransferWindow,
        winter: winterTransferWindow,
        absoluteDay: absoluteDay,
      ));
    }
  ...
  void _applyMatchConsequences({
    ...
    for (final performance in allPerformances) {
      final player = _playerIndex[performance.playerId];
  ...
  List<WorldEvent> processCareerMatchConsequences({
    ...
    final worldPlayer = _playerIndex[career.id];
  ...
  void processEndOfSeason({required int nextSeasonStartYear}) {
    ...
    // FinanceEngine is the single owner of the season-end club cashflow.
    // Do not apply a second seasonal income movement later in this method.
    ...
    // P1.1: retirement can create an impossible senior squad before the next
    // transfer window. Add a small emergency senior intake only when a club
    // falls below the playable minimum. The normal academy pipeline remains
    // responsible for long-term youth development.
    final emergencySenior = SquadDepthEngine.ensureMinimumSeniorSquads(
      clubs: clubs,
      players: players,
      generator: worldPlayerGenerator,
      seasonYear: nextSeasonStartYear,
    );
    for (final player in emergencySenior) {
      lastDayEvents.add(WorldEvent(
        year: nextSeasonStartYear,
        month: 7,
        day: 1,
        type: 'squad_depth',
        title: 'Uzupełnienie kadry',
        description: '${player.name} dołącza do ${_findClub(player.clubId ?? '')?.name ?? 'klubu'} po zakończeniu sezonu, aby utrzymać minimalną liczebność pierwszej kadry.',
        clubId: player.clubId,
        playerId: player.id,
        importance: 1,
      ));
    ...
    // Rebuild the persistent club roster index after emergency intake.
    // SquadDepthEngine mutates the player graph directly, so the roster cache
    // must be synchronized before later integrity checks and transfer logic.
    _syncClubRosters();
    ...
    for (final club in clubs) {
      ...
      for (final prospect in prospects) {
        ...
        lastDayEvents.add(WorldEvent(
          ...
          description: '${player.name}, ${player.age} lat, ${positionCodeOf(player.position)}, ${prospect.scoutingEstimateMin}-${prospect.scoutingEstimateMax} POT (pewność ${prospect.scoutingConfidence}%) trafia do akademii ${club.name}. Źródło: ${prospect.scoutingPath}, region: ${prospect.region}.',
          clubId: club.id,
    ...
    // YouthCareerEngine can legitimately move an 18-24 year old without a
    // professional contract back to the academy. That is correct for true
    // academy prospects, but it must not leave a club below the playable
    // senior minimum after the full end-of-season pipeline has completed.
    // Run the depth guard again after youth promotion/demotion and before the
    // next season is generated. This also repairs older saves where a senior
    // player was persisted without hasProfessionalContract.
    final postYouthSenior = SquadDepthEngine.ensureMinimumSeniorSquads(
      clubs: clubs,
      players: players,
      generator: worldPlayerGenerator,
      seasonYear: nextSeasonStartYear,
    );
    for (final player in postYouthSenior) {
      lastDayEvents.add(WorldEvent(
        year: nextSeasonStartYear,
        month: 7,
        day: 1,
        type: 'squad_depth',
        title: 'Końcowe uzupełnienie kadry',
        description: '${player.name} uzupełnia kadrę ${_findClub(player.clubId ?? '')?.name ?? 'klubu'} po zakończeniu procesów młodzieżowych.',
        clubId: player.clubId,
        playerId: player.id,
        importance: 1,
      ));
    ...
    // FinanceEngine already applied the complete season-end cashflow above.
    // Keeping a single financial owner prevents accidental double-crediting
    // of clubs every season.
  ...
  void _applyPromotionAndRelegation() {
    final polish = leagues.where((l) => l.country == 'Polska').toList();
    final byLevel = <int, List<League>>{};
    for (final league in polish) {
      byLevel.putIfAbsent(league.level, () => []).add(league);
    }

    void swapBetween(League upper, League lower, {int count = 2}) {
      final upperTable = tableForLeague(upper.id);
      final lowerTable = tableForLeague(lower.id);
      if (upperTable.length < count || lowerTable.length < count) return;
      final relegated = upperTable.reversed.take(count).map((s) => _findClub(s.clubId)).whereType<Club>().toList();
      final promoted = lowerTable.take(count).map((s) => _findClub(s.clubId)).whereType<Club>().toList();
      for (final club in relegated) club.leagueId = lower.id;
      for (final club in promoted) club.leagueId = upper.id;
    }

    final level1 = byLevel[1] ?? const <League>[];
    final level2 = byLevel[2] ?? const <League>[];
    final level3 = byLevel[3] ?? const <League>[];
    final level4 = byLevel[4] ?? const <League>[];

    if (level1.isNotEmpty && level2.isNotEmpty) swapBetween(level1.first, level2.first, count: 2);
    if (level2.isNotEmpty && level3.isNotEmpty) swapBetween(level2.first, level3.first, count: 2);

    if (level3.isNotEmpty && level4.length == 4) {
      final secondTable = tableForLeague(level3.first.id);
      if (secondTable.length >= 2) {
        final relegated = secondTable.reversed.take(2).map((s) => _findClub(s.clubId)).whereType<Club>().toList();
        final promoted = <Club>[];
        for (final group in level4) {
          final table = tableForLeague(group.id);
          if (table.isNotEmpty) {
            final club = _findClub(table.first.clubId);
            if (club != null) promoted.add(club);
          }
        }
        for (var i = 0; i < promoted.length && i < 4; i++) {
          promoted[i].leagueId = level3.first.id;
        }
        for (var i = 0; i < relegated.length; i++) {
          relegated[i].leagueId = level4[i % level4.length].id;
        }
      }
    }
  }
  ...
  void _syncClubRosters() {
    // Rebuild the O(1) indexes first: everything below (including the
    // roster rebuild itself) can then use map lookups instead of scanning
    // `clubs`/`players` linearly.
    _clubIndex = {for (final club in clubs) club.id: club};
    _playerIndex = {for (final player in players) player.id: player};
  ...
  // PERF: was `players.where((player) => player.clubId == clubId).toList()`
  // — a scan of the ENTIRE player pool (~1700 players) on every call, for
  // every club, every match, every day. club.playerIds is already kept in
  // sync by _syncClubRosters(), so this now only touches the ~25-30 players
  // actually on that club's roster.
  List<Player> _playersOfClub(String clubId) {
    final club = _clubIndex[clubId];
    if (club == null) return const [];
    return club.playerIds
        .map((id) => _playerIndex[id])
        .whereType<Player>()
        .toList();
  }

  // PERF: was a linear scan of the full club list on every call.
  Club? _findClub(String id) => _clubIndex[id];

  /// P0.7: validates the persistent player/club graph after a simulation tick.
  /// The report is diagnostic only and never mutates the world.
  WorldIntegrityReport validateWorldIntegrity() {
    _syncClubRosters();
    return WorldIntegrityValidator.validate(clubs: clubs, players: players);
  }

...
/// Extracts a stable, human-readable identifier from a position value.
///
/// Works whether [PlayerPosition] is a real Dart enum (using its built-in
/// `.name` getter) or, in older/divergent copies of this codebase, a plain
/// class with static const values (which has no `.name`). Never throws, so
/// a save can never be aborted just because of this lookup.
String positionCodeOf(dynamic position) {
  try {
    final dynamic n = position.name;
    if (n is String && n.isNotEmpty) return n;
  } catch (_) {}
  final s = position.toString();
...
class Player {
  ...
  int loanUntilDay;
  // Active-loan accounting. These fields make the loan promises real rather
  // than cosmetic: minutes, wage share and buyout terms survive save/load.
  int loanStartedDay;
  ...
  this.commercialEvents = 0,
    ...
    this.loanWageShare = 0.0,
    this.loanBuyoutClause = 0.0,
    this.loanFeePaid = 0.0,
    this.fatigue = 0,
    this.fitness = 100,
    this.form = 70,
    this.morale = 70,
  ...
  Map<String, dynamic> toJson() => {
    'id': id, 'name': name, 'age': age, 'position': positionCodeOf(position),
    ...
    'value': value, 'weeklyWage': weeklyWage, 'clubId': clubId, 'shirtNumber': shirtNumber,
    ...
    'loanStartedDay': loanStartedDay, 'loanStartMinutes': loanStartMinutes,
    'loanWageShare': loanWageShare, 'loanBuyoutClause': loanBuyoutClause, 'loanFeePaid': loanFeePaid,
  ...
  factory Player.fromJson(Map<String, dynamic> j) => Player(
    ...
    value: (j['value'] ?? 0).toDouble(), weeklyWage: (j['weeklyWage'] ?? 0).toDouble(), clubId: j['clubId'], shirtNumber: j['shirtNumber'] ?? 0,
    ...
    loanFeePaid: (j['loanFeePaid'] ?? 0).toDouble(), fatigue: j['fatigue'] ?? 0,

...
class PlayerCareer {
  ...
  int loanUntilDay;
  int loanStartedDay;
  int loanStartMinutes;
  double loanWageShare;
  double loanBuyoutClause;

  // Rozwój sezonowy: ogranicza skoki OVR i pozwala wynagradzać także minuty
  // oraz dobre występy, a nie tylko codzienny trening.
  int developmentSeasonYear;
  int seasonStartOverall;
  ...
  PlayerCareer({
    ...
    this.loanFromClubId,
    this.loanUntilDay = 0,
    this.loanStartedDay = 0,
    this.loanStartMinutes = 0,
    this.loanWageShare = 0,
    this.loanBuyoutClause = 0,
    this.developmentSeasonYear = 2026,
    this.seasonStartOverall = 60,
  ...
  void ensureDevelopmentSeason(int season) {
    if (developmentSeasonYear == season) return;
    developmentSeasonYear = season;
    seasonStartOverall = overall;
  }

  bool canGainFromDevelopment(int season) {
    ensureDevelopmentSeason(season);
    return overall < seasonStartOverall + developmentCapForAge() && overall < potential;
  }

  /// Występy mogą dać mały, rzadki impuls rozwojowy. OVR nadal pozostaje
  /// ograniczony do kilku punktów na sezon.
  bool applyPerformanceGrowth({required int season, required int minutes, required double rating, required int goals, required int assists}) {
    ensureDevelopmentSeason(season);
    if (!canGainFromDevelopment(season) || minutes < 30) return false;
    final quality = rating >= 8.0 || (rating >= 7.5 && (goals + assists) > 0) || (rating >= 7.7 && minutes >= 75);
    if (!quality) return false;

    final before = overall;
    final stat = switch (position) {
      PlayerPosition.goalkeeper => physical,
      PlayerPosition.defender => defending,
      PlayerPosition.midfielder => passing,
      PlayerPosition.winger => dribbling,
      PlayerPosition.striker => shooting,
    };
    if (stat >= potential) return false;
    final next = stat + 1;
    switch (position) {
      case PlayerPosition.goalkeeper: physical = next; break;
      case PlayerPosition.defender: defending = next; break;
      case PlayerPosition.midfielder: passing = next; break;
      case PlayerPosition.winger: dribbling = next; break;
      case PlayerPosition.striker: shooting = next; break;
    }
    refreshOverall();
    if (overall > seasonStartOverall + developmentCapForAge()) {
      switch (position) {
        case PlayerPosition.goalkeeper: physical = stat; break;
        case PlayerPosition.defender: defending = stat; break;
        case PlayerPosition.midfielder: passing = stat; break;
        case PlayerPosition.winger: dribbling = stat; break;
        case PlayerPosition.striker: shooting = stat; break;
      }
      overall = before;
      return false;
    }
    return overall > before;
  }

  factory PlayerCareer.fromJson(Map<String, dynamic> j) {
    ...
    contract: j['contract'] is Map ? PlayerContract.fromJson(Map<String, dynamic>.from(j['contract'])) : null,
    ...
    contractYearsRemaining: j['contractYearsRemaining'] ?? 1,
    loanFeeExpectation: (j['loanFeeExpectation'] ?? 0).toDouble(),
    guaranteedMinutesExpectation: j['guaranteedMinutesExpectation'] ?? 0,
    buyoutClauseExpectation: (j['buyoutClauseExpectation'] ?? 0).toDouble(),
    loanFromClubId: j['loanFromClubId'], loanUntilDay: j['loanUntilDay'] ?? 0,
    loanStartedDay: j['loanStartedDay'] ?? 0, loanStartMinutes: j['loanStartMinutes'] ?? 0,
    loanWageShare: (j['loanWageShare'] ?? 0).toDouble(), loanBuyoutClause: (j['loanBuyoutClause'] ?? 0).toDouble(),
    inMatchSquad: j['inMatchSquad'] ?? false,
    ...
    final stats = j['matchStats'];
    if (stats is Map) {
      player.matchStats.loadFromJson(Map<String, dynamic>.from(stats));
    }
    return player;
  }

...
class DailySimulationCore {
  ...
  DailySimulationReport runDay({
    ...
    required bool Function() hasCareerMatchToday,
    ...
    // Phase 4: a player fixture is a hard transaction boundary. The match
    // must be played in MatchScreen before any career/world consequences are
    // committed. Other league fixtures may still be resolved after that
    // interactive transaction is finalized.
    if (hasCareerMatchToday()) {
      phases.add(DailySimulationPhase.awaitingCareerMatch);
      final absoluteDay =
    ...
    // Phase 5: no player match today, so the normal autonomous-world tick can
